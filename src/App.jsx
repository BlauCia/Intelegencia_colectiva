...
const makeDefault = (name = "Sesión 1") => ({
  ...
  analysisPrompts: {
    1: "Analiza las respuestas sobre la experiencia. Destaca aspectos más mencionados e insights clave. Máximo 120 palabras. Sin asteriscos ni markdown.",
    2: "Analiza los aprendizajes destacados. Identifica conceptos que más han resonado. Máximo 120 palabras. Sin asteriscos ni markdown.",
    3: "Analiza cómo aplicarán lo aprendido. Identifica intenciones y patrones concretos. Máximo 120 palabras. Sin asteriscos ni markdown.",
  },
  ...
  sessionName: name, branding: { sessionTitle: "Inteligencia Colectiva" },
  anonymityMessage: "Tus respuestas son completamente anónimas.", sessionContext: "",
});
...
const createSession = async (name, copyFrom = null) => {
  ...
  const base = copyFrom
    ? { ...makeDefault(name), questions: copyFrom.questions, analysisPrompts: copyFrom.analysisPrompts, branding: copyFrom.branding, anonymityMessage: copyFrom.anonymityMessage, timerDuration: copyFrom.timerDuration, sessionContext: copyFrom.sessionContext || "" }
    : makeDefault(name);
...
const css = `
  ...
  *{-webkit-font-smoothing:antialiased}
  ...
  .qed{background:var(--dark3);border:1px solid var(--border);padding:1.25rem;margin-bottom:1px}
...
// ─── UTILS ────────────────────────────────────────────────────────────────────
// Safe countdown — always called, uses null-safe timerStart
...
// ─── CLAUDE API ───────────────────────────────────────────────────────────────
const callClaude = async (system, user, onChunk) => {
  try {
...
const callClaudeJSON = async (system, user) => {
  let full = "";
...
const runAnalysis = async (state, qNum, sessionId) => {
  ...
  let summary = "";
  await callClaude(
    `Eres analista de formación. ${analysisPrompts[qNum]}\nSin asteriscos, sin markdown.`,
    `Respuestas:\n\n${texts.join("\n---\n")}`,
  ...
  const enriched = await callClaudeJSON("Analista de feedback. SOLO JSON.",
  ...
  if (qNum < 3) {
    const nq = await callClaudeJSON("Facilitador experto. SOLO JSON.",
    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "var(--black)" }}>
      <div style={{ borderBottom: "1px solid var(--border)", padding: "1rem 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", gap: 4 }}>
          {[1, 2, 3].map(i => <div key={i} style={{ width: 36, height: 3, background: i < currentQuestion ? "var(--green)" : i === currentQuestion ? "var(--yellow)" : "var(--dark3)", transition: "background .3s" }} />)}
        </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "2rem 1.5rem", maxWidth: 640, margin: "0 auto", width: "100%" }}>
        <div style={{ fontSize: ".62rem", fontWeight: 700, letterSpacing: ".15em", textTransform: "uppercase", color: "var(--gray)", marginBottom: ".75rem" }}>Pregunta {currentQuestion} de 3</div>
        ...
        {submitted ? (
          ...
          <>
            ...
            <textarea value={text} onChange={e => setText(e.target.value)} placeholder="Añade un matiz (opcional)…"
              ...
              onFocus={e => e.target.style.borderColor = "var(--yellow)"} onBlur={e => e.target.style.borderColor = "var(--border)"} />
            <button className="btn by" style={{ width: "100%", justifyContent: "center", opacity: stance ? 1 : .4 }} onClick={submit} disabled={!stance}>
              Enviar reacción →
            </button>
          </>
        )}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div className="card">
        ...
        <div style={{ background: "var(--dark3)", border: "1px solid var(--yb)", padding: "1rem", marginBottom: "1.25rem" }}>
          ...
          <textarea value={genCtx} onChange={e => setGenCtx(e.target.value)} placeholder="Describe el contexto de la formación…" className="fta" style={{ minHeight: 68, marginBottom: ".75rem" }}
            onFocus={e => e.target.style.borderColor = "var(--yellow)"} onBlur={e => e.target.style.borderColor = "var(--border)"} />
        ...
        {questions.map((q, i) => (
          <div key={i} className="qed">
            ...
            <textarea value={q.text} onChange={e => { const qs = [...questions]; qs[i] = { ...qs[i], text: e.target.value }; setQuestions(qs); }} className="fta" style={{ minHeight: 56, marginBottom: ".6rem" }}
              onFocus={e => e.target.style.borderColor = "var(--yellow)"} onBlur={e => e.target.style.borderColor = "var(--border)"} />
        ...
        <div style={{ marginTop: "1rem" }}><button className="btn by bsm" onClick={saveQ}>Guardar preguntas</button></div>
      </div>
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          {[["Nombre de sesión", "sessionName", state.sessionName], ["Título de la plataforma", "branding.sessionTitle", state.branding?.sessionTitle || "Inteligencia Colectiva"], ["Mensaje de anonimato", "anonymityMessage", state.anonymityMessage]].map(([label, field, val]) => (
            <div key={field}>
              <label className="fl">{label}</label>
              <input defaultValue={val} className="fi" onBlur={e => {
                if (field.includes(".")) { const [a, b] = field.split("."); update({ [a]: { ...state[a], [b]: e.target.value } }); }
...
const PresenterDashboard = ({ initialSessionId }) => {
  ...
  // All hooks called unconditionally — safe defaults when state is null
  ...
  const { status, currentQuestion, responses, analysis, streamingText, wordFrequency,
    questions, analysisPrompts, sentiment, highlights, suggestedQuestions,
    resultsPublished, debateActive, debateQuestion, debateResponses, sessionName } = state;
  ...
  const closeDebate = async () => {
    ...
    let meta = "";
    await callClaude("Analista. Máx 80 palabras. Sin markdown.",
      `${counts.agree} de acuerdo, ${counts.nuance} matices, ${counts.disagree} desacuerdo. Meta-análisis.`,
      async c => { meta += c; await update({ [`debateAnalysis.q${debateQuestion}`]: meta }); });
  };
  const addDemo = () => {
    const demos = [
      { q1: "Muy inspiradora. Ha cambiado mi perspectiva sobre el liderazgo.", q2: "La escucha activa como base del liderazgo.", q3: "Cambiaré mis hábitos de comunicación en el equipo." },
      { q1: "Sorprendente. Conectar humanidades con empresa es revelador.", q2: "La creatividad como motor de decisiones.", q3: "Presencia y reflexión en mis conversaciones." },
    ...
    <div style={{ minHeight: "100vh", background: "var(--black)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "1.5rem", maxWidth: 1000, margin: "0 auto", width: "100%" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            <div className="card">
              ...
              <div style={{ display: "flex", gap: ".5rem", flexWrap: "wrap", alignItems: "center", marginBottom: isLive ? "1rem" : 0 }}>
                ...
                {isLive && currentQuestion < 3 && <button className="btn by" onClick={() => advance()} disabled={isProcessing}>Siguiente →</button>}
                {isLive && currentQuestion === 3 && <button className="btn br" onClick={() => advance(true)} disabled={isProcessing}>■ Cerrar y analizar</button>}
                {isProcessing && <div style={{ display: "flex", alignItems: "center", gap: ".5rem", fontSize: ".75rem", fontWeight: 700, letterSpacing: ".06em", textTransform: "uppercase", color: "var(--yellow)" }}>
                  <Spinner size={16} /> Analizando con Claude…
                </div>}
        ...
        {tab === "prompts" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            {[1, 2, 3].map(qNum => (
              <div key={qNum} className="card" style={{ padding: "1rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: ".6rem" }}>
                  <span style={{ fontSize: ".68rem", fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: "var(--gray)" }}>Pregunta {qNum}</span>
                  {editingPrompt === qNum ? (
                    <div style={{ display: "flex", gap: ".35rem" }}>
                      <button className="btn by bsm" onClick={() => { update({ [`analysisPrompts.${qNum}`]: promptDraft }); setEditingPrompt(null); }}>Guardar</button>
                      <button className="btn bg bsm" onClick={() => setEditingPrompt(null)}>Cancelar</button>
                    </div>
                  ) : (
                    <button className="btn bg bsm" onClick={() => { setEditingPrompt(qNum); setPromptDraft(analysisPrompts?.[qNum] || ""); }}>Editar</button>
                  )}
                </div>
                {editingPrompt === qNum ? (
                  <textarea value={promptDraft} onChange={e => setPromptDraft(e.target.value)} className="fta" style={{ minHeight: 85 }} />
                ) : (
                  <p style={{ fontSize: ".82rem", color: "rgba(255,255,255,.5)", lineHeight: 1.6 }}>{analysisPrompts?.[qNum]}</p>
                )}
              </div>
            ))}
          </div>
  ...
  if (status === "processing") return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--black)" }}>
      <div style={{ textAlign: "center" }}>
        <Spinner />
        <div className="bc" style={{ fontSize: "1.5rem", fontWeight: 900, textTransform: "uppercase", marginTop: "1rem" }}>Analizando</div>
      </div>
    </div>
  );
  return (
